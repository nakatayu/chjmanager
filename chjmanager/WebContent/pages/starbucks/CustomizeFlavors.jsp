...
<script type="text/javascript">
						...
						if (msg.status == 200) {
							...
							for ( var i = 0; i < msg.rows.length; i++) {
									...
									array.push('<td>'
											...
											" /><input name='appendId' id='appendType' type='hidden' value="+msg.rows[i]['isCkeckbox']+
												" /><button  type='button' onclick='addAttributeValue(this)' >+</button>"
													+ '</td>');
							}
	...
	function addAttributeValue(add) {
		var appendId = $(add).parent().find("input[id=appendId]").val(); //获取hidden里面的值
		var checkBoxType = $(add).parent().find("input[id=appendType]").val(); //获取hidden里面的值
		window.location = "addAttributeValue.jsp?appendId=" + appendId+"&checkBoxType="+checkBoxType;
	}
	// 增加属性名称 
