	...
	<div id="content">
		<div id="content-header">
			<div id="breadcrumb">
				...
				<i class="icon-home"></i> 主页</a> <a href="#" class="current">话题维护</a>
			</div>
		</div>
					...
					<div class="widget-box">
						...
						<div class="widget-title"
							...
							<div class="pagination alternate"
								...
								<button class="btn btn-warning" onclick="addTopic()">新&nbsp;&nbsp;&nbsp;&nbsp;增</button>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<button class="btn btn-warning" type=""
									onclick="deleteTopList()">删&nbsp;&nbsp;&nbsp;&nbsp;除</button>
							</div>
						</div>
...
<script type="text/javascript">
					...
					success : function(msg, status) {
						...
						if (msg.status == 200) {
							for ( var i = 0; i < msg.rows.length; i++) {
								...
								array
										.push('<td>'
												+ '<input type="checkbox" value="' + data["topic_id"]+ '"/>'
												+ '</td>');
								array.push('<td>' + data["topic_name"]
										+ '</td>');
								array.push('<td>' + data["summary"] + '</td>');
								array.push('<td>'+ data["create_time"]+ '</td>');
								array.push('<td>' + data["tp"] + '</td>');
								array.push('<td>' + data["c"] + '</td>');
								array.push('<td>' +tStatus[(data["examine_result"]) - 1] + '</td>');
								array.push('<td>' +TopStatus[(data["status"])-1] + '</td>');
								array.push('<td>'+ '<a href="javascript:void(0)" onclick="updateTopic('+ data["topic_id"] + ')">'+ "修改" + '</a>' + '</td>');	
							
								array.push("<td>"	+"<div class='btn-group'>"
							 			+"<button data-toggle='dropdown' class='btn btn-warning dropdown-toggle' style=''>审核<span class='caret'></span></button>"
							 			+"<ul class='dropdown-menu' >"
							 			+"<li><a href='javascript:void()' onclick='updateNTopInfo("+data["topic_id"]+","+data["status"]+")'>通过</a></li>"
							 			+"<li><a href='javascript:void()' onclick='updateNoTopInfo("+data["topic_id"]+","+data["status"]+")'>驳回</a></li>"
							 			+"</ul></div>" 
										+ "</td>"); 
								array.push('</tr>');
							}
		...
		if (confirm("您确认删除此记录吗？")) {
			...
			if ($("#table_content").find("input[type=checkbox]:checked").size() == 0) {
				...
				$.ajax({
					type : "post",
					data : {
						selectedTopicIds : label
					},
					url : '${ctx}/topic!deleteTopList.action',
					dataType : 'json',
					success : function(msg, status) {
						if (msg.status == 200) {
							alert("删除成功！");
							window.location = "TopicList.jsp";
						}
					}
				});
			}
	...
	function addtopicinfo() {
		location.href = "${ctx}/pages/company/TopicInfo.jsp";
	}
	function updateTopic(topic_id) {

		window.location = "${ctx}/topic!updateTop.action?topic_id=" + topic_id;
	}
	function addTopic() {
		location.href = "${ctx}/pages/company/addTopicInfo.jsp";
	}
	//操作话题
	function updateTopInfo(topic_id,sta){
				$.ajax({
				type : "post",
				url : '${ctx}/topic!updateTstatus.action',
				dataType : 'json',
				data : {
					topicId : topic_id,
					TopStatus : sta,
	...
	/* function updateNoTopInfo(topic_id){
		$.ajax({
			type : "post",
			url : '${ctx}/topic!updateNoTopInfo.action',
			dataType : 'json',
			data : {
				topicId : topic_id,
			},
	...
	function updateNTopInfo(topic_id){	
		 $.ajax({
				type : "post",
				url : '${ctx}/topic!updateNTopInfo.action',
				dataType : 'json',
				data : {
					topicId:topic_id,
					status:status,
