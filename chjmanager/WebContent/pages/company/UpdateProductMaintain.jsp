				...
				<form class="form-horizontal" method="post" name="basic_validate"
					...
					<div id="form-wizard-1" class="step">
						...
						<div class="control-group">
							...
							<div class="controls">
								<table>
									<tr>
										<td><label> <c:if
													test="${productInfo.time_type==1}">
													<input type="radio" id="timeType" name="timeType" value="1"
														checked />
												</c:if> <c:if test="${productInfo.time_type!=1}">
													<input type="radio" id="timeType" name="timeType" value="1" />
												</c:if> 立即生效
										</label></td>
									</tr>
									<tr>
										<td><label> <c:if
													test="${productInfo.time_type==2}">
													<input type="radio" id="timeType" name="timeType" value="2"
														checked />
												</c:if> <c:if test="${productInfo.time_type!=2}">
													<input type="radio" id="timeType" name="timeType" value="2" />
												</c:if> 手动设定
										</label></td>
										<td><label class="control-label">设定时间：</label>
											<div class="controls">
												...
												<input style="width: 170px" type="text" id="endTime"
													...
													value="${productInfo.end_time}"
													onfocus="WdatePicker({minDate:'#F{$dp.$D(\'startTime\');}',dateFmt:'yyyy-MM-dd'})" />
											</div></td>
									</tr>
									<tr>
										<td><label><c:if
													test="${productInfo.time_type==3}">
													<input type="radio" id="timeType" name="timeType" value="3"
														checked />
												</c:if> <c:if test="${productInfo.time_type!=3}">
													<input type="radio" id="timeType" name="timeType" value="3" />
												</c:if> 下架 </label></td>
									</tr>
								</table>
						...
						<div class="form-actions" style="padding: 20px 0 100px 50px;">
							<a onclick="saveProductDetail()" class="btn btn-warning"
								style="margin: 5px 30px;">保&nbsp;&nbsp;&nbsp;&nbsp;存</a> <a
								onclick="moreAttributeUpdate()" class="btn btn-warning"
								style="margin: 5px 30px;">更多属性</a> <a class="btn btn-warning"
								onclick="window.history.go(-1);" style="margin: 5px 30px;">返&nbsp;&nbsp;&nbsp;&nbsp;回</a>
						</div>
					</div>
				</form>
			</div>
