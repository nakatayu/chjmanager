					...
					<div class="widget-box">
						...
						<div class="widget-content nopadding">
							<form class="form-horizontal" method="post"
								action="${ctx}/activity!updateActivityInfo.action" name="basic_validate"
							...
							<br>
									<table>
																...
																<label class="control-label">活动标题：</label>
																	<div class="controls">
																		<input id="title" name="title" type="text"
																			style="width: 254px;" onblur="titleOnBlur()"  value="${activity.activity_title}"/> <span><font
															...
															<div class="control-group">
																...
																<div class="controls">
																	<input id="summaryId" type="text" name="summary"
																		style="width: 250px;" onblur="summaryIdOnBlur()" value="${activity.summary}"/> <span><font
															...
															<div class="control-group">
																...
																<div class="controls">
																	<input id="address" type="text" name="address"
																		style="width: 250px;" onblur="addressOnBlur()" value="${activity.address}"/> <span><font
															...
															<div class="control-group">
																...
																<div class="controls">
																	<input id="userMax" type="text" name="userMax"
																		style="width: 250px;" onblur="userMaxOnBlur()"value="${activity.user_max}" /> <span><font
																		color="red" id="userMax_check"> </font></span>
																		<input  id="acimg" name="acimg" type="hidden" />
																</div>
															</div>
														</div>	
											...
											<input style="width: 170px" type="text"
												id="beginTime" name="beginTime" value="${activity.begin_time}"
													class="Wdate" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'endTime\');}',dateFmt:'yyyy-MM-dd HH:mm'})"/>&nbsp;&nbsp;至&nbsp;&nbsp;
											<input style="width: 170px" type="text" id="endTime"
										name="endTime" value="${activity.end_time}" class="Wdate" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'beginTime\');}',dateFmt:'yyyy-MM-dd HH:mm'})"/>
										</div>	
															...
															<div class="control-group">
																	...
																	<div class="controls">
																		<table>
																			<tr>
																				<td width="80"><label><c:if test="${activity.is_check=='T'}">
																				 <input type="radio" id="deliverOk" name="deliverOk" value="T" checked="checked" />是 
																				 </c:if>
																				 <c:if test="${activity.is_check!='T'}">
																				 <input type="radio" id="deliverOk" name="deliverOk" value="T" />是 
																				 </c:if>
																				</label></td>
																				<td><label> <c:if test="${activity.is_check=='F'}">
																				<input type="radio"id="deliverOk" name="deliverOk" value="F"  checked="checked"/>否 
																				</c:if>
																				<c:if test="${activity.is_check!='F'}">
																				<input type="radio"id="deliverOk" name="deliverOk" value="F" />否 
																				</c:if>
																				</label>
																...
																<div class="control-group">
																	...
																	<div class="controls">
																		<table>
																			<tr>
																				<td width="80"><label>
																				<c:if test="${activity.join_type=='T'}">
																				<input type="radio" id="deliverOk1" name="deliverOk1" value="T" checked="checked"/>线上
																				</c:if>
																				<c:if test="${activity.join_type!='T'}">
																				<input type="radio" id="deliverOk1" name="deliverOk1" value="T" />线上
																				</c:if>
																				</label></td>
																				<td><label>
																				<c:if test="${activity.join_type=='F'}">
																				<input type="radio" id="deliverOk1" name="deliverOk1" value="F" checked="checked"/>线下
																				</c:if>
																				<c:if test="${activity.join_type!='F'}">
																				<input type="radio" id="deliverOk1" name="deliverOk1" value="F" />线下
																				</c:if>
																				</label>
																			</td>
																		</tr>
																...
																<div class="control-group">
																	...
																	<div class="controls">
																		<table>
																			<tr>
																				<td width="80"><label> 
																				<c:if test="${activity.is_qrcode=='T'}">
																				<input type="radio" id="deliverOk2" name="deliverOk2" value="T" checked="checked"/> 是 
																				</c:if>
																				<c:if test="${activity.is_qrcode!='T'}">
																				<input type="radio" id="deliverOk2" name="deliverOk2" value="T"/> 是 
																				</c:if>
																				</label></td>
																				<td><label> 
																				<c:if test="${activity.is_qrcode=='F'}">
																				<input type="radio" id="deliverOk2" name="deliverOk2" value="F" checked="checked"/> 否 
																				</c:if>
																				<c:if test="${activity.is_qrcode!='F'}">
																				<input type="radio" id="deliverOk2" name="deliverOk2" value="F"/>否
																				</c:if>
																				</label>
																				<input id="actId" name ="labelValue" style="display: none;">
																				<input id="activid" name="ActivityId" style="display: none;">
																			</td>
																		</tr>
																	</table>
																</div>
													...
													<div class="shop_announcement">
														...
														<div class="controls" style="height: 200px">
															<textarea type="text" name="content"
																...
																style="height: 150px" id="content11" >${activity.activity_content} </textarea>
	                                  	...
	                                  	<h5>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;活动标签：</h5>
											...
											<div class="control-group" style="height: 68px;">
												...
												<c:forEach items="${activityTag }"  var="tag">
												...
												<div class="form-actions">
													<button class="btn btn-warning" type="button"
														class="btn btn-success" onclick="updateActivith()">修&nbsp;&nbsp;&nbsp;&nbsp;改</button>
													&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
													<button class="btn btn-warning" type="button"
...
<script type="text/javascript">
	$(function() {
		...
		new AjaxUpload($uploadBtn, {
			...
			name : 'upfile',
			responseType : 'json',
			type:5,//个人头像1企业所有图片2店铺所有图片3圈子图片4话题图片5活动图片6商品图片7广告图片8个人证件图片9二维码图片10其他图片11
			...
			onComplete : function(file, response) {
				if (response.status == 200) {
					...
					$("#upa").append('<img src="${http_img}'+  response.rows+'"  style="margin-left:20px;"  width="100" height="100"/>');
				} else {
					alert(response.msg);
				}
			}
...
// 修改活动信息
	...
	//人数上限验证
		...
		$('#activid').val(ActivityId);
		$('#acimg').val("${activity.activity_image}");
			submit();
	window.location = "${ctx}/pages/company/ActivityList.jsp";
}
	...
	new AjaxUpload($uploadBtn, {
		...
		onComplete : function(file, response) {
			if (response.status == 200) {
				$("#uploadUrl").val(response.rows);
				$("#uaty").attr("src", "${http_img}" + response.rows);
			} else {
				alert(response.msg);
			}
		}
