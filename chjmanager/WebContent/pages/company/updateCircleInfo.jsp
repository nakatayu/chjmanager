					...
					<div class="widget-box">
						...
						<div class="widget-content nopadding">
							...
							<form class="form-horizontal" method="post"
								action="${ctx}/circle!updateCirleInfo.action"
											...
											<div id="form-wizard-1" class="step">
												...
												<div class="controls">
													<input id="name" name="name" type="text"
														style="width: 254px;" onblur="nameOnBlur()" value="${circle.name }" /> <span><font
											...
											<div class="control-group">
												...
												<div class="controls">
													<table>
														<tr>
															<td width="170"><label>
															<c:if test="${circle.is_check=='T' }">
															 <input type="radio" id="checkbox" name="checkbox" value="T" checked="checked"/>是否审核（是）
															 </c:if>
															 <c:if test="${circle.is_check!='T' }">
															 <input type="radio" id="checkbox" name="checkbox" value="T" />是否审核（是）
															 </c:if>
															</label></td>
															<td><label> 
															<c:if test="${circle.is_check=='F' }">
															<input type="radio" id="checkbox" name="checkbox" value="F" checked="checked"/>是否审核（否）
															</c:if>
															<c:if test="${circle.is_check!='F' }">
															<input type="radio" id="checkbox" name="checkbox" value="F" />是否审核（否）
															</c:if>
															</label></td>
														</tr>
													</table>
									...
									<div class="control-group">
										...
										<div class="controls">
											<input id="CircleAbstract" type="text" name="CircleAbstract"
												style="width: 250px;" onblur="CircleAbstractOnBlur()" value="${circle.summary }" /> <span><font
								...
								<div class="shop_announcement">
									...
									<div class="controls" style="height: 200px">
										<textarea type="text" name="info" class="shop_announcement"  onblur="content11OnBlur()"
											style="height: 100px" id="content11" >${circle.circle_content }</textarea>
										...
										<input id="updateCId" name="circleId" style="display: none;">
										<input id="ccid" name="ccid" type="hidden">
									</div>
								</div>
						</div>
						<!-- 		<br>
						...
						<div class="form-actions">
							<button class="btn btn-warning" type="button"
								class="btn btn-success" onclick="updateCirleInfo()">修&nbsp;&nbsp;&nbsp;&nbsp;改</button>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<button class="btn btn-warning" type="button"
...
<script type="text/javascript">
		...
		new AjaxUpload($uploadBtn,
				{
					...
					onComplete : function(file, response) {
						...
						//	$("#loadImgeid").attr("src","${http_img}" + response.rows);
							$("#upcl").append('<img src="${http_img}'+  response.rows+'"  style="margin-left:20px;"  width="100" height="100"/>');
							} else {
							alert(response.msg);
						}
					}
				...
				.ajax({
					...
					success : function(msg, status) {
						if (msg.status == 200) {
							var array = [];
							for ( var i = 0; i < msg.rows.length; i++) {
								array.push('<li class="tag_content"	onclick="selected(this)" onmouseover="this.id=\'a1\' " onmouseout="this.id=\'this\'"');
								array.push('	 value="' + msg.rows[i]["tagId"]
										+ '">' + msg.rows[i]["tagName"]
										+ '</li>')
							}
	...
	function updateCirleInfo() {
		...
		var labelValue = "";
		var circleId =
<%=request.getParameter("circle_id")%>
	;
		$("#choosenLabel").find("span").each(function() {
			labelValue = labelValue + $(this).attr("value") + ",";
		...
		$('#updateCId').val(circleId);
		$('#ccid').val("${circle.circle_image}");
	//提交更新   	
		if(flag!=false){
			submit();
		}
