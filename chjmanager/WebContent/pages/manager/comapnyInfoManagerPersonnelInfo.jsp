		...
		<div id="content-header">
			<div id="breadcrumb">
				<a href="${ctx}/pages/index.jsp" title="主页" class="tip-bottom"><i
					...
					href="${ctx}/pages/manager/companyInfoManager.jsp" title="企业列表"
					class="current"> <i class=" icon-fire"></i>企业列表
				</a> <a href="javascript:void()" onclick="companyPersonnel()"
					class="current"> <i class=" icon-edit"></i>人员账号列表
				</a> <a href="#" class="current">新增人员账号</a>
			</div>
		</div>
					...
					<div class="widget-box">
						...
						<form class="widget-content nopadding" id="formDivId">
									...
									<table width="25%">
										<tr>
											<c:if test="${param.type==3}">
												<td><label id="company">企业 <input type="radio"
														onclick="disabledCloseSend()" id="deliverOk" name="type"
														value=2 />
												</label></td>
												<td><label id="shop"> 店铺 <input type="radio"
														onclick="disabledOpenSend()" id="deliverNo" name="type"
														value=3 /> <span  style="margin-left: 20px;"><font color="red"
															id="personnelType_check">*必选</font></span>
												</label></td>
											</c:if>
											<c:if test="${param.type==2}">
												<td><label id="compangPersonnelId">企业 <input
														type="radio" onclick="xinbakeCloseSend()" id="closeId"
														name="type" value=4 />
												</label></td>
												<td><label id="shopPersonnelId"> 店铺 <input
														type="radio" onclick="xinbakeOpenSend()" id="openId"
														name="type" value=5 /> <span style="margin-left: 20px;"><font color="red"
															id="xinbakeType_check">*必选</font></span>
												</label></td>
											</c:if>
											<c:if test="${param.type==1}"><td colspan="2">
											<label>开发区<input name="type" value=1 type="hidden"/></label></td></c:if>
										</tr>
									</table>
								</div>
							</div>
							...
							<input name="organizationalID" id="organizationalID"
								type="hidden"> <input name="companyId" id="companyId"
								type="hidden" value="${param.companyId}">
						</form>
					</div>
				</div>
			</div>
...
<%=request.getParameter("companyId")%>
	...
	$(function() {
		...
		$.ajax({
			type : "post",
			url : '${ctx}/shop!findCompanyShopInfo.action?companyId='
					+ companyId,
			dataType : 'json',
			timeout : 10000,
	...
	function xinbakeCloseSend() {
		var flagOk = $("#closeId").is(":checked");
		if (flagOk == true) {
			$("#disabledOutSend").hide();
		}
	}
	function xinbakeOpenSend() {
		var flagNo = $("#openId").is(":checked");
		if (flagNo == true) {
	...
	//返回企业人员账号管理列表
	function companyPersonnel(companyId) {
		window.location.href = "${ctx}/pages/manager/comapnyInfoManagerPersonnelList.jsp?companyId="
				+
<%=request.getParameter("companyId")%>
	;
	}
