			...
			<div class="span12">
				<div class="widget-box">
					<div  align="left" class="widget-title">
							<input style="margin: 3px 0px 2px 5px" align="right" type="text" id="txtName" placeholder="请输入正式名称"  onblur="loadData(1)">
					...
					<div class="widget-title"
						...
						<div class="row-fluid" style="margin: 6px 0px 0px 0px">
							<button class="btn btn-warning" onclick="addPersonnel()">新增人员</button>
							&nbsp;&nbsp;&nbsp;&nbsp;
							<button class="btn btn-warning" onclick="delPersonnel()">删除人员</button>
						    &nbsp;&nbsp;&nbsp;&nbsp;
...
function loadData(page, size, name) {
	...
	$.ajax({
		...
		success : function(msg, status) {
			var array = [];
			type=msg.themeCount["type"];
			if (msg.status == 200) {				
				for(var i=0; i<msg.rows.length;i++){
					var data = msg.rows[i];
					 array.push('<tr>');
					 array.push('<td>'+ '<input type="checkbox" value="' +  data["id"] + '"/>'+ '</td>');
					 array.push('<td>' + '<a href="javascript:void(0)" onclick="findPersonnel('+data["id"]+','+ data["companyId"]+','+ data["organizationalID"]
						+ ')">'+data["name"] + '</td>');
					 array.push('<td>' +'<a href="javascript:void(0)" onclick="findPersonnel('+data["id"]+','+ data["companyId"]+','+ data["organizationalID"]
						+ ')">' +data["userName"] + '</td>');
...
function addPersonnel(){
	 location.href = "${ctx}/pages/manager/comapnyInfoManagerPersonnelInfo.jsp?companyId="+<%=request.getParameter("companyId")%>+"&type="+type;
}
//查询账号
function findPersonnel(personnelId,companyId,organizationalID){
	location.href = "${ctx}/personnel!findPersonnelInfo.action?personnelId="+personnelId+"&companyId="+companyId+"&organizationalID="+organizationalID;
}
//删除账号
...
function setPernelManager(){
	...
	window.location.href='${ctx}/personnel!seltRoleOfPersonnel.action?personnelId='+lbl[0]+'&type='+3;	
}
