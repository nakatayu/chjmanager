...
<div id="content">
		<div id="content-header">
			<div id="breadcrumb">
				 ...
				 <a  href="javascript:void(0)"  onclick="window.history.go(-1)" class="current">
				 <i class=" icon-fire"></i>店铺列表</a>
				<a href="#" class="current">店铺订单</a>
			</div>
		</div>
...
var companyShopId = <%=request.getParameter("companyShopId")%>;
$("#companyShopId").val(companyShopId);
	...
	function loadData(page, size, name) {
		...
		pageinfo["companyShopId"] = $('#companyShopId').val();
		  $.ajax({
					type : "post",
					...
					success : function(msg, status) {
						...
						if (msg.status == 200) {
							for ( var i = 0; i < msg.rows.length; i++) {
								...
								array.push("<td><a class='oederNumCss'  href='javascript:void(0)' onclick='orderInfo("+data["orderId"]+","+data["status"]+")'>" +data["flowNo"]+ "</a></td>");
							...
							$('#pager').html('');
							$('#orderList').html('<tr><td colspan="4">'+msg.msg+'</td></tr>');
						}
					}
				}); 
	}
	...
	function orderInfo(orderId,status){
		location.href = "${ctx}/order!findOrderInfo.action?orderId="+orderId+"&status="+status+"&companyShopId="+companyShopId+"&companyId="+<%=request.getParameter("companyId")%>;
	}
	...
	//返回店铺列表
	function shopList(companyId){
		location.href = "${ctx}/pages/manager/companyInfoManagerShopList.jsp?companyId="+<%=request.getParameter("companyId")%>;
	}
</script>	
