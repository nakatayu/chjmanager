					...
					<table class="table table-bordered table-striped">
						...
						<div class="widget-title"
						  ...
						  <div class="pagination alternate"
								...
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<button class="btn btn-warning" onclick="findPush(3)">审核通过</button>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<button class="btn btn-warning" onclick="findPush(2)">审核不通过</button>
								</div>
					    </div>
					</div>
				</div>
...
<script type="text/javascript">
					...
					success : function(msg, status) {
						...
						if (msg.status == 200) {
							for ( var i = 0; i < msg.rows.length; i++) {
								...
								if(data["manner"]=='-1'){array.push('<td id="pushType'+data["push_id"]+'" value="'+data["manner"]+'">全部</td>');}
								if(data["manner"]=='1'){array.push('<td id="pushType'+data["push_id"]+'" value="'+data["manner"]+'">短信</td>');}
								if(data["manner"]=='2'){array.push('<td id="pushType'+data["push_id"]+'" value="'+data["manner"]+'">App</td>');}
								
								array.push('<td>' + data["username"]+ '</td>');
								array.push('<td>' + data["apply_time"]+ '</td>');
								
								if(data["status"]=='0'){array.push('<td id="status'+data["push_id"]+'">未审核</td>');}
								if(data["status"]=='1'){array.push('<td id="status'+data["push_id"]+'">审核中</td>');}
								if(data["status"]=='2'){array.push('<td id="status'+data["push_id"]+'">审核未通过</td>');}
								if(data["status"]=='3'){array.push('<td id="status'+data["push_id"]+'">审核通过</td>');}//1 审核中  2 审核未通过 3 审核通过 
	...
	PageClick = function(pageclickednumber) {
	 ...
	 //查看 
	 var status='',pushId='',pushType='';
	function  findPush(statusParam){
		status=statusParam;
		...
		if ($("#table_content").find("input[type=checkbox]:checked").size() == 0) {
			alert("请选择一条数据!");
				...
				$("#table_content").find("input[type=checkbox]:checked").each(
						function() {
							if($('#status'+$(this).val()).html()=='审核通过'){
								flag=true;
							}
							pushType = $('#pushType'+$(this).val()).attr("value");
				...
				if(!flag){
					...
					pushId=label[0];
					showPopWin('审核意见','${ctx}/pages/manager/pushCheck.jsp', 600, 300,checkResult);	
				}
				else{
					alert('已审核通过!');
				}
	...
	function checkResult(type,result){
	if(type==1){
		hidePopWin(false);
		showPopWin('正在处理','${ctx}/pages/manager/loading.jsp', 600, 100);
		checkPush(result);
	}
	else{
		hidePopWin(false);
	...
	function checkPush(result){
		 $.ajax({
				type : "post",
				url : '${ctx}/push!updatePushInfoStatus.action',
				dataType : 'json',
				data:{pushType:pushType,checkContent:result,status:status,pushIds:pushId},
