				...
				<div class="span12">
																							...
																							<table width="100%">
																								<c:forEach items="${action2.next}" var="action3">
																									<tr valign="top">
																										<td
																											style="padding: 5px 0 0 10px; border-top: 1px #330033 solid;"><c:if
																												test="${action3.check==1&&action3.select==1}">
																												<input type="checkbox" value="${action3.ID}"
																													checked="checked" name="action"
																													parentID="${action3.ParentID}"
																													onclick="checkAction(this)" />
																											</c:if> <c:if
																												test="${action3.check==1&&action3.select==0}">
																												<input type="checkbox" value="${action3.ID}"
																													name="action"
																													parentID="${action3.ParentID}"
																													onclick="checkAction(this)" />
																											</c:if> <c:if
																												test="${action3.check==0&&action3.select==1}">
																												<input type="checkbox" value="${action3.ID}"
																													checked="checked" readonly="readonly"
																													disabled="disabled" name="action"
																													parentID="${action3.ParentID}" />
																											</c:if> <c:if
																												test="${action3.check==0&&action3.select==0}">
																												<input type="checkbox" value="${action3.ID}"
																													readonly="readonly" disabled="disabled"
																													name="action"
																													parentID="${action3.ParentID}" />
																											</c:if> ${action3.Action} <c:if
					...
					<div
						...
						<div class="btn-group" style="float: left; padding-left: 200px;">
							...
							<button class="btn btn-warning" style="margin: 0 20px 0 20px;"
								onclick="window.location.href='${ctx}/personnel!seltRolesAction.action?type=0'">角色维护</button>
