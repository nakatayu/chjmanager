		...
		<div class="container-fluid">
			<div class="row-fluid">
					<div class="widget-box" style="margin-bottom: 0px;">
						...
						<input style="margin: 3px 0px 2px 5px" align="right" type="text" id="txtName" placeholder="请输入商品附加属性名称"  onblur="loadData(1)">
...
<script type="text/javascript">
					...
					success : function(msg, status) {
						if (msg.status == 200) {
							...
							for ( var i = 0; i < msg.rows.length; i++) {
									...
									array	.push('<input type="checkbox"  value="'+msg.rows[i]["child"][h]["appendId"]+'"/>'+msg.rows[i]["child"][h]["name"]);
									}
									array.push('</td>');
									array.push('<td>'
													+ "<input name='appendId' id='appendId' type='hidden' value="+msg.rows[i]['appendId']+" />"
													+ " <button type='button' class='btn btn-info btn-mini' onclick='addAttributeValue(this)' >添加属性值</button>"
													+ '</td>');
							}
	...
	function addAttributeValue(add) {
		...
		window.location = "compnayInfoManagerAddProductAttributeValue.jsp?appendId=" + appendId;
	}
	// 增加属性名称 
	function addAttributeName() {
		window.location = "compnayInfoManagerAddProductAttribute.jsp";
	}
	//删除属性  父属性 
				...
				$("#customizeFlavors").parent().find(
				 ...
				 $.ajax({
							...
							success : function(msg, status) {
								if (msg.status == 200) {
									alert("删除成功！");
									window.location = "compnayInfoManagerProductAttribute.jsp";
								}
							}
						}); 
			}
