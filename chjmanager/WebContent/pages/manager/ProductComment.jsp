	...
	<div id="content">
		<div id="content-header">
			<div id="breadcrumb">
				...
				<i class="icon-home"></i> 主页</a> <a href="#" class="current">商品评论维护</a>
			</div>
		</div>
...
<td align="left" style="font-size: 20px;font-weight: bold;">商品评论列表：</td>  
...
function updatCommentDelete(){
	...
	$.ajax({
		type : "post",
		url : '${ctx}/product!updtProductCommentl.action',
		dataType : 'json',
		data : {produCommId:str,productId:productId},
...
function loadData(page) {
	 ...
	 pageinfo["page"]=page;
	  ...
	  $.ajax({
		type : "post",
		url : '${ctx}/product!findProductCommenst.action',
		dataType : 'json',
		data : pageinfo,
		success : function(msg, status) {
			if(msg.status==200){
				...
				for(var i=0;i<msg.rows.length;i++){
					var data=msg.rows[i];
					str+='<tr><td><input type="checkbox" name="comBoc" value="'+data["product_comment_id"]+'"/></td>';
					var name=data["user"];
				...
				$('#produCommlst').html('<tr><td colspan="4">'+msg.msg+'</td></tr>');
				$('#pager').html('');
			}			
		}				
   }); 
