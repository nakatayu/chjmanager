...
<script src="/chjmanager/js/My97DatePicker/WdatePicker.js"></script>
							...
							<form id="basic_validate" class="form-horizontal">
								...
								<table width="100%">
									<tr>
										...
										<td rowspan="6">
											<table>
												...
												<tr>
													<td style="padding-left: 30px; color: red;">图片上传尺寸：290*170px
														限制上传：300KB</td>
												</tr>
												<tr>
											...
											<div class="control-group" style="width: 280px;">
												...
												<div class="controls">
													<input type="text" id="address" name="address"
														onblur="addressOnBlur()" style="width: 250px;" /> <span><font
											...
											<div class="control-group">
												...
												<div class="controls">
													<input type="text" id="linkmanTel" name="linkmanTel"
														onblur="linkmanTelOnBlur()" style="width: 250px;" /> <span><font
									...
									<tr>
										<td>
											<div class="control-group">
												<label class="control-label">企业星级：</label>
												<div class="controls">
													<input type="text" id="starScore" name="starScore"
														style="width: 250px;" /> <span><font color="red"
														id="starScore_check"> </font></span>
												</div>
											</div>
										</td>
										<td></td>
												...
												<div class="control-group">
													...
													<div class="controls">
														<textarea id="companyInfo" name="companyInfo"
															style="width: 720px; height: 200px"></textarea>
													</div>
												</div>
											</div>
										</td>
...
<script type="text/javascript">
			...
			.ajax({
				...
				success : function(msg, status) {
					if (msg.status == 200) {
						var array = [];
						for ( var i = 0; i < msg.rows.length; i++) {
							array.push('<li class="tag_content"	onclick="tagname2(this)" onmouseover="this.id=\'a1\' " onmouseout="this.id=\'this\'"');
	...
	function selectLabelName(li) {
		...
		if (flag == true) {
			...
			$("#choosenLabel")
					.append(
							"<span   style='font-size:16px'  value="
									+ $(li).attr("value")
									+ ">"
									+ $(li).text()
									+ "</span><a style='font-size:16px' onclick='delLabel(this)'>[X]&nbsp;&nbsp;</a>");
	...
	function saveCompany() {
		...
		var starScore = $.trim($("#starScore").val());//企业名称
		if (starScore > 5) {
			$("#starScore_check").html("企业星级数最高为5");
			flag = false;
		} else if (!reg.test(starScore)) {
			$("#starScore_check").html("只能为数字!");
			flag = false;
		} else {
			$("#starScore_check").html("");
		}
		...
		if (flag != false) {
			...
			$.ajax({
				type : "post",
				url : '${ctx}/companyinfomanager!newCompany.action',
				dataType : 'json',
				data : data,
				success : function(msg, status) {
					if (msg.status == 200) {
						alert("保存成功");
						window.location = "companyInfoManager.jsp";
					} else {
						alert("保存失败");
					}
				}
