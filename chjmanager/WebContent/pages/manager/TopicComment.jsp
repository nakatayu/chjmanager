	...
	<div id="content">
		<div id="content-header">
			<div id="breadcrumb">
				...
				<i class="icon-home"></i> 主页</a> <a href="#" class="current">话题评论维护</a>
			</div>
		</div>
...
<td align="left" style="font-size: 20px;font-weight: bold;">话题评论列表：</td>  
...
<td align="left"><button class="btn btn-success" onclick="loadnewData(10)" style="font-size: 20px;cursor: pointer;">查询</button>
     ...
     <div class="widget-box"  style="margin-right: 40px;">
            ...
            <table class="table table-bordered table-striped with-check" >
              ...
              <tbody id="topicCommentlist" style="word-break:break-all; word-wrap:break-word;" >
...
function updatCommentDelete(){
	...
	$.ajax({
		type : "post",
		url : '${ctx}/topic!updatCommentDelete.action',
		dataType : 'json',
		data : {topicCommentId:str,topicId:'${param.topicId}'},
...
function loadData(page) {
	 pagenumber=page;
	 pageinfo["topicId"]="${param.topicId}";
	 ...
	 pageinfo["page"]=page;
	  ...
	  $.ajax({
		type : "post",
		url : '${ctx}/topic!findTopicCommentList.action',
		dataType : 'json',
		data : pageinfo,
		success : function(msg, status) {
			if(msg.status==200){
				...
				for(var i=0;i<msg.rows.length;i++){
					var data=msg.rows[i];
					str+='<tr><td><input type="checkbox" name="comBoc" value="'+data["topic_comment_id"]+'"/></td>';
					var name=data["user"];
				...
				$('#topicCommentlist').html('<tr><td colspan="4">'+msg.msg+'</td></tr>');
				$('#pager').html('');
			}			
		}				
   }); 
